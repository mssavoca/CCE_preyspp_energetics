...
ggsave("Fig. 4_EW_plot_v8.pdf", summary_plot_uncorrected_facet, width = 10, height = 6)
...
EDvsLength_uncorrected <- ggplot(combustion_df, aes(x=LENGTH.mm., y=ENERGY_DENSITY.kJ.g.WET.)) +
  geom_point(aes(color = SPECIES, shape = as.factor(year))) +
...
ggsave("Fig. 5_EDvsMorphoPlot_v2.pdf", Fig_5_EDvsMorphoPlot, width = 13, height = 11.25)
...
EDvsTIME_facet <- ggplot(filter(combustion_df, month_name !="NA"), aes(x = month_name, y = ENERGY_DENSITY.kJ.g.WET.)) +
  geom_boxplot(outlier.shape = NA) + 
  geom_jitter(aes(color = SPECIES, shape = as.factor(year)),
              alpha=0.4, width = 0.2) +
...
ggsave("Fig. 6_EDvsTIME_facet_v8.pdf", EDvsTIME_facet, width = 12, height = 10.25)
