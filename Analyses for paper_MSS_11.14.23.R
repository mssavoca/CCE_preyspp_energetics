...
Ew_WetDry_glmm <- lmer(ENERGY_DENSITY.kJ.g.WET. ~ wet_dry_ratio + (1|month_name) + (1|collection), 
                data = 
...
Ew_glmm <- lmer(ENERGY_DENSITY.kJ.g.WET. ~ EW_sp + (1|month_name) + (1|year), 
                data = 
                  #filter(combustion_df, SPECIES %in% c("E. mordax", "S. sagax"))
                  combustion_df_for_analy
                )
...
ED_glmm <- lmer(ENERGY_DENSITY.kJ.g.WET.AFDW. ~ EW_sp + (1|month_name) + (1|year), 
                data = 
                  #filter(combustion_df, SPECIES %in% c("E. mordax", "S. sagax"))
                  combustion_df_for_analy
)
...
Ew_length_glmm <- lmer(ENERGY_DENSITY.kJ.g.WET. ~ LENGTH.mm. + (1|month_name), 
                       data = 
                      #combustion_df, SPECIES == "E. pacifica/ T. spinifera")
...
Ew_length_glmm_squid <- lmer(ENERGY_DENSITY.kJ.g.WET. ~ LENGTH.mm. + (1|month_name), 
                       data = filter(combustion_df, SPECIES == "D. opalescens")
)
summary(Ew_length_glmm_squid)



Ew_wt_glmm_squid <- lmer(ENERGY_DENSITY.kJ.g.WET. ~ (WW/num_individuals) + (1|month_name), 
                   data = filter(combustion_df, SPECIES == "D. opalescens")
)
summary(Ew_wt_glmm_squid)
